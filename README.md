...
- Keep Harness bound to `127.0.0.1`.
...
Using HTTPS makes the remote UI a secure context and avoids patching individual Web Crypto call sites.
...
2. **HTTP secure-context 问题 / Plain HTTP secure-context issues.**
   Some browser APIs such as `crypto.randomUUID()` require a secure context. HTTPS should be treated as mandatory for remote LAN use.
...
This repository currently contains deployment notes and configuration examples only. No upstream DeepSeek Harness source code is redistributed here.
